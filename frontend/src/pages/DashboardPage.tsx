...
import type { Almacen, Cliente, CompraLista, Inventario, Producto, VentaLista } from '../api/client'
...
export function DashboardPage() {
  ...
  useEffect(() => {
    Promise.all([salesApi.ventas(), purchasingApi.compras(), inventoryApi.inventario(), inventoryApi.productos(), inventoryApi.almacenes(), salesApi.clientes()])
      .then(([v, c, i, p, a, clientes]) => { setSales(v.items); setPurchases(c.items); setStock(i); setProducts(p); setWarehouses(a); setClients(clientes) })
      .catch((error: Error) => setMessage(error.message))
  }, [])

  const activeSales = sales.filter(x => x.estado !== 'ANULADA')
  const activePurchases = purchases.filter(x => x.estado !== 'ANULADA')
  const salesIncome = activeSales.reduce((sum, x) => sum + x.total, 0)
  ...
  return <AppLayout title="Resumen del negocio">
    <div className="dashboard-heading"><div><p className="eyebrow">Panel general</p><p className="dashboard-subtitle">Ventas, compras e inventario en un solo lugar.</p></div><a className="primary-link" href="/ventas/nueva">+ Nueva venta</a></div>
    {message && <p className="message">{message}</p>}
    <section className="metric-grid" aria-label="Indicadores principales">
      <article className="metric-card metric-primary"><span>Ingresos por ventas</span><strong>{money.format(salesIncome)}</strong><small>{activeSales.length} ventas vigentes</small></article>
      ...
      <article className="metric-card"><span>Flujo bruto</span><strong>{money.format(salesIncome - purchaseExpense)}</strong><small>Ventas menos compras registradas</small></article>
