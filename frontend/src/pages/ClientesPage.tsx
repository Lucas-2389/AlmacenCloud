...
export function ClientesPage() {
  ...
  return <AppLayout title="Clientes">
    <div className="toolbar"><label className="form-field">Buscar cliente<input value={search} onChange={e => setSearch(e.target.value)} placeholder="Documento o nombre" /></label><button onClick={load}>Buscar</button></div>
    <form className="grid-form" onSubmit={submit}>
      <label className="form-field">Tipo de documento<select value={form.tipoDocumento} onChange={e => update('tipoDocumento', e.target.value)}>{['DNI', 'RUC', 'CE', 'OTRO'].map(x => <option key={x}>{x}</option>)}</select></label>
      <label className="form-field">Número de documento<input placeholder="Ej. 12345678" value={form.numeroDocumento} onChange={e => update('numeroDocumento', e.target.value)} required /></label>
      <label className="form-field">Nombre o razón social<input placeholder="Nombre completo" value={form.nombreRazonSocial} onChange={e => update('nombreRazonSocial', e.target.value)} required /></label>
      ...
      <label className="form-field">Correo electrónico<input type="email" placeholder="correo@ejemplo.com" value={form.email} onChange={e => update('email', e.target.value)} /></label>
      <button>{id ? 'Actualizar cliente' : 'Crear cliente'}</button>
    </form>
    {message && <p className="message">{message}</p>}<table><thead><tr><th>Documento</th><th>Cliente</th><th>Contacto</th><th></th></tr></thead><tbody>{items.map(x => <tr key={x.id}><td>{x.tipoDocumento} {x.numeroDocumento}</td><td>{x.nombreRazonSocial}</td><td>{x.telefono} {x.email}</td><td><button onClick={() => edit(x)}>Editar</button> <button onClick={() => remove(x)}>Desactivar</button></td></tr>)}</tbody></table>
  </AppLayout>
}
