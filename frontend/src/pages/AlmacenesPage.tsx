...
export function AlmacenesPage() {
  ...
  return <AppLayout title="Almacenes"><form className="inline-form" onSubmit={submit}><input placeholder="Código" value={codigo} onChange={e => setCodigo(e.target.value)} required /><input placeholder="Nombre" value={nombre} onChange={e => setNombre(e.target.value)} required /><input placeholder="Dirección" value={direccion} onChange={e => setDireccion(e.target.value)} /><button>{id ? 'Actualizar' : 'Crear'}</button></form>{message && <p className="message">{message}</p>}<table><thead><tr><th>Código</th><th>Nombre</th><th>Dirección</th><th></th></tr></thead><tbody>{items.map(x => <tr key={x.id}><td>{x.codigo}</td><td>{x.nombre}</td><td>{x.direccion}</td><td><button onClick={() => edit(x)}>Editar</button> <button onClick={() => remove(x)}>Desactivar</button></td></tr>)}</tbody></table></AppLayout>
}
